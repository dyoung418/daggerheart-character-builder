...
- **At the table** (`play.html?id=<character>`, linked as "Play") — the character on a phone during a session, styled after the [Foundryborne](https://github.com/Foundryborne/daggerheart) Daggerheart sheet (MIT; fonts under the OFL in `assets/fonts/`): name, level and heritage, Hope diamonds you tap, the class domains, the six traits on shields (with the tier mark), then tabs — **Status** (HP and Stress pips, Evasion, Armor shields, Proficiency, thresholds, experiences), **Weapons**, **Cards** (the loadout's full text) and **Features**. Tap a pip to mark up to it, tap a marked one to clear it and the ones after. The marks are saved with the character; the maxima always come from the current sheet, so a level up or an armor change resizes the rows.

...
- **At the table** (`play.html?id=<character>`, linked as "Play") — the character on a phone during a session: Evasion and damage thresholds up top, then HP, Stress, Hope and Armor as rows of boxes you tap to mark and clear (tap a box to fill up to it, tap a marked box to clear it and the ones after). The marks are saved with the character; the maxima always come from the current sheet, so a level up or an armor change resizes the rows.

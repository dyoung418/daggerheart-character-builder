...
- **Card browser** (`index.html`) — filter the domain cards by domain, type and level, or search them by name or by what they actually say; build a 5-card loadout plus a vault, saved locally. Cards are listed by domain, then by level, then by name, and each one appears once — if you have more than one edition of the SRD switched on, you see the newer wording. **Export cards (PDF)** prints whatever the filters are showing, so you can get a whole domain's cards as cards: 9 to a US Letter page at the real 2.5 × 3.5 in, with cut marks to cut along. Print at **100%**, not "fit to page".
